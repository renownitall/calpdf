...
class TestExportTocCLI:
    ...
    def test_export_tree_format(self, sample_pdf_with_toc: Path):
        result = runner.invoke(
            app, ["export-toc", str(sample_pdf_with_toc), "--format", "tree"]
        )
    ...
    def test_export_tree_ignores_output_file(
        self, sample_pdf_with_toc: Path, tmp_path: Path
    ):
...
class TestApplyTocCLI:
    ...
    def test_full_roundtrip(self, sample_pdf_with_toc: Path, tmp_path: Path):
        ...
        for orig, rt in zip(original_toc, roundtrip_toc, strict=True):
