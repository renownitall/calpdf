...
class TestEntryPoint:
    """The console entry points must stay importable and runnable."""

    def test_python_m_calpdf_version(self):
        result = subprocess.run(
            [sys.executable, "-m", "calpdf", "--version"],
            capture_output=True,
            text=True,
            timeout=60,
        )
        assert result.returncode == 0
        assert "calpdf" in result.stdout
