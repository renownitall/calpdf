...
from calpdf.prune import remove_targets_that_would_dangle
...
class Job:
    ...
    def target_pdf(self) -> Path:
        ...
        assert self.output_pdf is not None  # guaranteed by in_place's definition
...
def target_page_geometry(pdf: pikepdf.Pdf, mode: Mode, pages: int) -> PageGeometry:
    ...
    # Sample the first page that will remain after the replaced pages are
    # removed, so the cover matches the body page size. When every page is
    # being replaced there is no remaining page to sample, so fall back to
    # the last existing page instead of failing.
    idx = min(pages, len(pdf.pages) - 1) if mode == Mode.replace else 0
...
def _try_passthrough_jpeg(
    image_path: Path, pixel_w: int, pixel_h: int
) -> Optional[bytes]:
    """Return the original JPEG bytes when they can be embedded unchanged.

    Avoids a lossy decode/resize/encode round trip when the source is already
    an RGB JPEG with no EXIF rotation and exactly the computed pixel size.
    Returns None whenever any conversion would be required.
    """
    try:
        with Image.open(image_path) as img:
            if (img.format or "").upper() != "JPEG":
                return None
            if img.mode != "RGB":
                return None  # grayscale/CMYK needs conversion
            if img.size != (pixel_w, pixel_h):
                return None
            orientation = img.getexif().get(0x0112)
            if orientation not in (None, 0, 1):
                return None  # exif_transpose would alter the pixels
...
def build_cover_pdf(
    ...
    jpeg_data = _try_passthrough_jpeg(image_path, pixel_w, pixel_h)
    if jpeg_data is None:
        if fit_mode == FitMode.match_width:
            final_img = img_rgb.resize((pixel_w, pixel_h), Image.Resampling.LANCZOS)
...
def set_cover(
    ...
    from calpdf.dlcover import download_cover, validate_book_id
