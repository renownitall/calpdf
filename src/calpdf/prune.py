...
When pages are removed from a PDF, any outline entries, internal links,
named destinations, or open actions that point at the removed pages would
dangle.  This module collects and removes those targets so page deletion
leaves a structurally clean document.
"""
...
class RemovedTargets:
    ...
    def annotation_removed(self, annot) -> bool:
        ...
        if "/Dest" in annot and self.destination_removed(annot["/Dest"]):
            return True
        return "/A" in annot and self.action_removed(annot["/A"])
    ...
    def outline_item_removed(self, item) -> bool:
        ...
        if isinstance(raw, pikepdf.Dictionary):
            if "/Dest" in raw and self.destination_removed(raw["/Dest"]):
                return True
            return "/A" in raw and self.action_removed(raw["/A"])
