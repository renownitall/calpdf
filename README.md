...
Calibre is good at organizing books, but it's the wrong tool for editing PDFs. calpdf works on the PDFs directly, so you don't need a Calibre library to use it. A tool like `qpdf` can replace a cover page, but you'd have to match it to the width of the book's pages yourself. The jobs that keep coming up are simple, like finding a cover, swapping it in, shrinking a PDF, or editing bookmarks without installing a PDF editor. calpdf keeps each one to a single command.
